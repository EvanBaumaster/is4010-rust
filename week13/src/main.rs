...
pub fn analyze_text(text: &str) -> (usize, f64, String) {
    ...
    (word_count, avg_word_length, longest_word.to_string())
}
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ParseError::NotANumber => write!(f, "The input is not a number"),
            ParseError::NotPositive => write!(f, "The number is not positive"),
        }
    }
}
